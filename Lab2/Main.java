...
class FullTimeStaff extends Staff {
    private double dailySalary; // 每天的工资
    private double overtimePay; // 加班每小时工资
...
// 主程序类（Main 类替代 PayrollSystem）
public class Main {
    public static void main(String[] args) {
        // 如果没有传入文件名和员工类型，则提示使用方法
        if (args.length != 2) {
            System.out.println("使用方法: java Main <文件名> <员工类型 (fulltime/parttime)>");
            return;
        }
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(fileName))) {
            int count = Integer.parseInt(br.readLine().trim()); // 读取第一行，表示有多少个员工
